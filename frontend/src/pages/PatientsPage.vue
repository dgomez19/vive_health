<template>
  <q-page>
        ...
        <q-form ref="form" @submit="submit" @reset="reset">
          <div class="row">
            <div class="col-5 col-md-5 col-xs-12">
              <q-input outlined v-model="document" label="Documento *" hint="Ingrese el número de documento del paciente" lazy-rules :rules="[obligatorio]" />
            </div>
          </div>
    ...
    <div class="q-pa-md">
      ...
      >
        <template #top>
          <q-btn push rounded rippe no-caps size="md" label="Agregar paciente" color="primary" icon="add" @click="openAdd"/>
          <q-space />
        </template>
...
const load = async (props) => {
  ...
  try {
    const { data } = await api.get(`appointment/patient/?${params}`)
...
const update = async () => {
  ...
  try {
    await api.put(`appointment/patient/${uuid.value}/`, {
      document: document.value,
      names: names.value,
      surnames: surnames.value,
      cell_phone: cellPhone.value,
...
const onDelete = async (uuid) => {
  $q.dialog({
    ...
    api.delete('appointment/patient/' + uuid + '/').then(_response => {
